package de.comeight.crystallogy.particles;

public class ParticleInformation {
	//-----------------------------------------------Variabeln:---------------------------------------------
	public static final String ID_PARTICLE_A = "crystallogy:a";
	public static final String ID_PARTICLE_B = "crystallogy:b";
	public static final String ID_PARTICLE_C = "crystallogy:c";
	public static final String ID_PARTICLE_D = "crystallogy:d";
	public static final String ID_PARTICLE_E = "crystallogy:e";
	public static final String ID_PARTICLE_F = "crystallogy:f";
	
	public static final String ID_PARTICLE_N_COLOR = "crystallogy:n_color";
	
	public static final String ID_PARTICLE_LIGHT = "crystallogy:light";
	public static final String ID_PARTICLE_INFUSION = "crystallogy:infusion";
	public static final String ID_PARTICLE_INFUSER_BLOCK_STATUS = "crystallogy:inferBlockstatus";
	public static final String ID_PARTICLE_ENDERON = "crystallogy:enderon";
	public static final String ID_PARTICLE_DEBUG = "crystallogy:debug";
	
	//-----------------------------------------------Constructor:-------------------------------------------

	
	//-----------------------------------------------Set-, Get-Methoden:------------------------------------

	
	//-----------------------------------------------Sonstige Methoden:-------------------------------------

	
}
